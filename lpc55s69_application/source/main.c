...
//	APP_MP3_Play(0);
    ...
    while(1)
    {
    	...
    	if(g_SystemStatus == 1)
    	{
            ...
            /* Fingerprint Task */
            if (FPS_WorkModeGet() == FPS_ENROLLMENT_MODE)
            {
                APP_FPS_Enroll_Task(g_FingerEnrollNum);
            }
            if (FPS_WorkModeGet() == FPS_VERIFY_MODE)
            {
                g_FPSTaskStatus = APP_FPS_Verify_Task();
            }
            ...
            {
                APP_MOTOR_Task(MOTOR_UNLOCK);                   /* Operate unlock system */
            ...
            {
                APP_MOTOR_Task(MOTOR_LOCK);                     /* Operate lock system */
                g_MATTERTaskStatus = MATTERTASKIDLE;            /* Clean up Status */
                g_AliveTicks = SLEEPTIME*OSINTERVAL;            /* System Active 10 seconds */
                APP_MP3_Play(33);
            }
            ...
            {
                APP_MOTOR_Task(MOTOR_UNLOCK_AND_LOCK);          /* Operate unlock and lock system */
