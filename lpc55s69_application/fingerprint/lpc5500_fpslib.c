...
uint8_t FPS_TemplateWrite(PBL_TEMPLATE pblTemplate, uint8_t *fp_id)
{
    uint32_t flash_addr;
    uint8_t template_num = 0;
    uint8_t first_valid_pos = 0xFF;

    for (uint8_t i = 0; i < g_OptGlobal.StoreFingers; i++)
    {
        flash_addr = g_flashAddress + (i * g_OptGlobal.FingerFlashSize);

        uint8_t flag = *(uint8_t *)(flash_addr);
        if (flag == 0xF0)
        {
            template_num++;
        }
    ...
    *fp_id = first_valid_pos;
    flash_addr = g_flashAddress + (first_valid_pos / 8 * 32768);

    if (template_num < g_OptGlobal.StoreFingers)
    {
		...
		{       
            ...
            memcpy((void *)&g_FPFlashBuf[(first_valid_pos % 8) * 4096], (uint8_t *)ptr, 4096);     /*  */
...
uint8_t FPS_TemplateDelete(uint8_t id)
{
    uint32_t addr;

    if (g_OptGlobal.FingerFlashSize != 4096)
    {
#if USE_FINGERPRINT
        FPS_PRINTF("!!! Only Support 4K Template!\r\n");
#endif
        return 0xF3;
    }

    addr = g_flashAddress + (id / 8 * 32768);
    memcpy((void *)g_FPFlashBuf, (uint8_t *)addr, 32768);
    memset((void *)&g_FPFlashBuf[(id % 8) * 4096], 0xFF, 4096);
    ...
    statusFlash = FLASH_Erase(&flashInstance, addr, 32768, kFLASH_ApiEraseKey);
    statusFlash = FLASH_VerifyErase(&flashInstance, addr, 32768);
    if (statusFlash != kStatus_Success)
    {
#if USE_FINGERPRINT
        FPS_PRINTF("!!! FLASH Verify Erase Failed!\n");
...
#if USE_FINGERPRINT
    FPS_PRINTF("!!! FLASH_Program Addr 0x%x Size %d!\r\n", addr, 32768);//OptGlobal.FingerFlashSize);
#endif
    statusFlash = FLASH_Program(&flashInstance, addr, g_FPFlashBuf, 32768);//OptGlobal.FingerFlashSize);
#if USE_FINGERPRINT
    /* Verify programming by reading back from flash directly */
    FPS_PRINTF("!!! Read Addr 0x%x Size %d!\r\n", addr, 32768); // OptGlobal.FingerFlashSize);
#endif
...
uint8_t FPS_VerifyTask(uint8_t *fp_id, uint32_t *TimeCost)
{
...
uint8_t FPS_RegistTask(uint8_t *fp_id, uint16_t *finger_times, uint16_t *finger_total)
{
    ...
    if (g_fingerWorkMode.mode == BTL_ENROLLMENT_MODE)
    {
        ...
        if (ret == BTL_RC_OK)
        {
            ...
            if (g_BTLResult == BTL_RC_OK)
            {
                ...
                {
                    if (FPS_TemplateWrite(&blMultiTemplates, fp_id) == 0)
                    {
#if USE_FINGERPRINT
