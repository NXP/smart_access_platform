...
import com.smartaccessmanager.model.BLEDevice;
...
public class MainActivity extends BaseScanActivity {
    ...
    public static Handler wakeUpHandler = new Handler();
    private static final int WAKE_UP_DELAY = 60 * 1000; /* time in ms */
        ...
        wakeUpHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                if (BLEService.INSTANCE.getConnectionState() == BLEService.State.STATE_CONNECTED)
                    BLEService.INSTANCE.getProtocol().sendWakeCall();
                wakeUpHandler.postDelayed(this, WAKE_UP_DELAY);
            }
        }, WAKE_UP_DELAY);
    ...
    class BLEDeviceAdapter extends RecyclerView.Adapter<BLEDeviceHolder> {
        ...
        public void onBindViewHolder(final BLEDeviceHolder holder, final int position) {
            ...
            holder.name.setText(TextUtils.isEmpty(device.getName()) ? "Unknown" : device.getName() +
                    " (" + (device.getBondState() == BluetoothDevice.BOND_BONDED ? "Bonded" : "Unbonded")+ ":" + bleDevice.getRssi() + "dB)");
