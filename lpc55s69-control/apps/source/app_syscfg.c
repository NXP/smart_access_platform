...
{
    static uint32_t i, num, user_index;
 ...
 * @return
 */
uint32_t APP_SYS_VIZNIDDelete(uint8_t vizn_id)
{
    uint32_t i, j;
    uint8_t target_id = vizn_id;

    for(i=0; i<50; i++)
    {
    	if(g_UserCfg.valid[i] == 0xAA) // valid User
    	{
            if(g_UserCfg.vizn_id[i] == vizn_id) // Match VIZN id
            {
            	g_UserCfg.vizn_id[i] = 0xFF; // Clean this id
    ...
    for(i=0; i<50; i++)
    {
    	if(g_UserCfg.valid[i] == 0xAA)
    	{
    		if(g_UserCfg.vizn_id[i] != 0xFF)
    		{
				if(g_UserCfg.vizn_id[i] > vizn_id)
				{
					g_UserCfg.vizn_id[i] = g_UserCfg.vizn_id[i]-1;
...
void APP_SYS_Format(uint8_t reset)
{
    ...
    if(reset == 0)
    {
        PRINTF("*** APP_SYS_Format Format SysCfg Informations!\r\n");
        memset((uint8_t *)TempBuf, 0xFF, 32768);                                                                                                                                                                                                              /* ���Buffer����ʼ��һ�� */
        status = FLASH_Erase(&flashInstance, USER_CFGADDR, 32768, kFLASH_ApiEraseKey);  
        status = FLASH_VerifyErase(&flashInstance, USER_CFGADDR, 32768);
        status = FLASH_Program(&flashInstance, USER_CFGADDR, TempBuf, 32768);
