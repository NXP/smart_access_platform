...
volatile uint16_t   g_fingerEnrollIndex;
...
void FPS_TimerInit(void)
{
    ...
    MRT_StartTimer(MRT0, kMRT_Channel_0, USEC_TO_COUNT(5000U, mrt_clock));       /* Start channel 0, 1ms*/
}
 ...
 * @return
 */
uint8_t APP_FPS_Task(uint16_t* index)
{
    volatile uint8_t retValue;
    volatile uint32_t s_FPSTimeCost;
    
    retValue = FPS_RegistTask( (uint16_t *)index, (uint16_t *)&g_FPSEnrollTimes, (uint16_t *)&g_FPSEnrollTotal);
    if(retValue == FPS_REGOK)
    {
        APP_MP3_Play(12);
        FPS_WorkModeSet(FPS_VERIFY_MODE);
        PRINTF("*** Enroll ID %d Successfully\r\n", g_fingerEnrollIndex);
        APP_BLE_Printf("AT+FINGERPRINT=OK\r\n");
    }
    if(retValue == FPS_REGFAILED)
    {
        APP_MP3_Play(13);
        FPS_WorkModeSet(FPS_VERIFY_MODE);
    ...
    if(retValue == FPS_REGING)
    {
        APP_MP3_Play(10);
        PRINTF("*** ID%d, Accept %d, Count %d\r\n", g_fingerEnrollIndex, g_FPSEnrollTimes, g_FPSEnrollTotal);
        PRINTF("*** Pls Place Your Finger again\r\n");
    }
    retValue = 0x00;
    
    retValue = FPS_VerifyTask((uint16_t *)index, (uint32_t *)&s_FPSTimeCost);
    if(retValue == FPS_IMAGEVALID)
    {
        PRINTF("*** Finger Status %x Index %d Time %d \r\n", retValue, *index, s_FPSTimeCost);
        PRINTF("*** Verify ID %d Success, %d mS\r\n", *index, s_FPSTimeCost );
    }
