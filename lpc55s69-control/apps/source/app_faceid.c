...
/**************************** registration  **************************************/
	...
	if(g_FACEIDCmdCmp != NULL)
	{
		...
		APP_SYS_VIZNIDDelete(g_BLEFaceIDdeletNum); // delete the 1st one,
		APP_BLE_Printf("%s", g_FACEIDCmdCmp);
		return 0;
	}
	...
	if(g_FACEIDCmdCmp != NULL)
	{
		// TODO : MP3 play
		// TODO: need delete in database
		APP_SYS_VIZNIDDelete(g_BLEFaceIDdeletNum);
		PRINTF("&&& AT+FACEDEL=SUCCESS\r\n");
		APP_BLE_Printf("AT+FACEDEL=SUCCESS\r\n");
		return 0;
	}
	...
	if(g_FACEIDCmdCmp != NULL)
	{
		...
		APP_BLE_Printf("AT+FACEDEL=FAIL\r\n");
		return 0;
	}
