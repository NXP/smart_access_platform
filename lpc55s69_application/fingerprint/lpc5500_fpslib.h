...
extern uint8_t  FPS_TemplateDelete(uint8_t id);

extern void     FPS_WorkModeSet(uint8_t data);
extern uint8_t  FPS_WorkModeGet(void);
extern uint8_t  FPS_VerifyTask(uint8_t *fp_id, uint32_t *TimeCost);
extern uint8_t  FPS_RegistTask(uint8_t *fp_id, uint16_t *finger_times, uint16_t *finger_total);
